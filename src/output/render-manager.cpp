...
extern "C"
{
    /* wlr uses some c99 extensions, we "disable" the static keyword to
     * workaround */
#define static
...
struct postprocessing_manager_t
{
    ...
    void run_post_effects()
    {
        ...
        {
            /* The last postprocessing hook renders directly to the screen,
             * others to
  ...
  public:
        ...
        output(o)
    {
        ...
        on_frame.set_callback([&] (void*)
        {
            ...
            // We cannot really wait less than 1ms, render right away in that
            // case
            if (total < 1)
            {
                paint();
