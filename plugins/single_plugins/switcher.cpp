...
class WayfireSwitcher : public wf::plugin_interface_t
{
  ...
  public:
    ...
    wf::gesture_callback touch_activate = [=] (wf::touchgesture_t*)
    {
        if (!active)
        {
            /* We set it to -1 to indicate that the user hasn't done anything
             * yet */
            touch_total_dx = -1;
    ...
    /* Sets up basic hooks needed while switcher works and/or displays
     * animations.
    ...
    void dearrange()
    {
         ...
         * it really is.  * To circumvent this, we just fade out one of the
         * copies */
