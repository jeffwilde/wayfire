    ...
    const double fast_threshold = 24)
{
    ...
    const bool too_right = vx == vw - 1 &&
        (accumulated_dx < 0.0 || last_deltas < 0.0);

    if (!too_left &&
        ((accumulated_dx > move_threshold) || (last_deltas > fast_threshold)))
    {
        target_dx = 1;
    } else if (!too_right &&
               ((accumulated_dx < -move_threshold) ||
                (last_deltas < -fast_threshold)))
    {
        target_dx = -1;
    }
