 ...
 *     move X Y | resize W H | (un)set fullscreen | (un)set maximized | set
 * alpha A
 *
 *    where (T) is a text surrounded by parenthesis, for ex. (tilix)
 *    contains (T) means that (T) can be found anywhere in the title/app-id
 * string
 *
...
class wayfire_window_rules : public wf::plugin_interface_t
{
    ...
    rule parse_add_rule(std::string rule)
    {
        ...
        /* first condition is so that there is no underflow in unsigned
         * arithmetic */
        ...
        } else if (starts_with(action, "set alpha"))
        {
            ...
            a = std::max(std::min(1.0f, a), 0.1f); /* clamp a in range [0.1f,
                                                    * 1.0f] */
